                    ...
                    if (loginType == "USER")
                    {
                        ...
                        var res = new MobileAppResponse<SCL_Mobile_Complaints>()
                        {
                            all = userComplaints.AsEnumerable().Select(s => new SCL_Mobile_Complaints
                            {
                                ...
                                img_data = s.SCL_ComplaintImages.Where(i => i.Complaint_ID == s.ID).FirstOrDefault() != null ? "https://scall-mobile.scm.azurewebsites.net//Attachments/" + s.SCL_ComplaintImages.Where(i => i.Complaint_ID == s.ID).FirstOrDefault().AttachmentName : "",
